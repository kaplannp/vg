...
FROM intel/oneapi-vtune:2024.1.0-devel-ubuntu22.04 AS base
MAINTAINER vgteam
...
RUN . ./source_me.sh && make -j $THREADS deps
...
RUN rm -f obj/version.o && . ./source_me.sh && make -j obj/version.o
...
#zkn removed make static
RUN . ./source_me.sh && make -j
